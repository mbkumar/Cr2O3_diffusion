...
for dfct_name in dc:
    ...
    plt.plot(temp_invs, np.log10(diff_coeff), lw=2, ls=ls[dfct_name], color=next(colors))
        #plt.plot(temp_invs, np.log10(dc[dfct_name][q]), lw=2, ls=ls[dfct_name], color=colors[abs(q)])
    legends.append(get_legend(dfct_name))
...
plt.savefig('Cr2O3_diff_vs_T_log_def_specie_P-{}.eps'.format(press))
plt.close()
